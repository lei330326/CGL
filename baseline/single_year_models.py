...
class SingleYearCNN(nn.Module):
    def __init__(self, args):
        ...
        self.n_m = 14 # Original: 14, new: 52*96, This includes management vars for ALL crops.
...
class SingleYearRNN(nn.Module):
    ...
    def forward(self, X):
        ...
        # X_w = X[:, :self.n_w]
        X_w = X[:, :self.n_w].reshape(-1, self.num_weather_vars, self.time_intervals) # [, num_weather_vars, time_intervals]
        ...
        # Process soil data
        X_s = X[:, self.n_w:self.n_w+self.n_s].reshape(n_batch, self.num_soil_vars, self.soil_depths)
