...
def train_epoch(args, model, device, nodeloader, year_XY, county_avg, year_avg_Y, cur_step, optimizer, epoch, writer=None):
    ...
    for year in all_years:  # list(year_XY.keys()):
        ...
        for batch_idx, (in_nodes, out_nodes, blocks) in enumerate(nodeloader):
            ...
            # print("in_nodes=",in_nodes.shape)
            # print("out_nodes=",out_nodes.shape)
...
def train(args):
    ...
    #print("year_XY1980-2018",year_XY[1980][0].shape)
    #exit(1)
    ...
    param_setting = "cnn-gar-lstm_bs-{}_lr-{}_maxepoch-{}_sche-{}_T0-{}_etamin-{}_step-{}_gamma-{}_dropout-{}_testyear-{}-encoder-{}".format(
        args.batch_size, args.learning_rate, args.max_epoch, args.scheduler, args.T0, args.eta_min, args.lrsteps, args.gamma, args.dropout, args.test_year,args.encoder_type)
   ...
   # exit(1)
    out_dim = 1
    model = GAT_LSTM(args, in_dim, out_dim).to(device)
